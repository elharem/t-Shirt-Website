@extends('layouts.app')
@section('title', 'TEE/SHOP — T-shirts uniques')
    ...
    <div class="container mx-auto px-4 py-8 md:py-12 grid md:grid-cols-2 gap-12 items-center">
        <div>
            ...
            <div class="flex gap-4 flex-wrap">
                ...
                <a href="{{ route('categories.index') }}" class="btn-outline">Catégories</a>
            </div>
        </div>
        <div class="relative">
...
<section class="container mx-auto px-4 py-16">
    <div class="flex justify-between items-end mb-10">
        ...
        <a href="{{ route('categories.index') }}" class="text-sm uppercase tracking-widest underline">Voir tout</a>
    </div>
    ...
    <a href="{{ route('categories.show', $cat) }}"
       class="group relative aspect-[3/4] bg-ink overflow-hidden block">
        ...
        <div class="relative h-full flex flex-col justify-end p-4 text-cream">
            <span class="text-xs opacity-60">{{ $cat->products_count }} produit{{ $cat->products_count > 1 ? 's' : '' }}</span>
            <h3 class="text-2xl font-display group-hover:text-accent transition">{{ $cat->name }}</h3>
        </div>
        <div class="absolute top-2 right-2 text-cream text-xs font-mono opacity-50">0{{ $i + 1 }}</div>
    </a>
@endforeach
    </div>
</section>
...
<section class="container mx-auto px-4 py-16">
    <div class="flex justify-between items-end mb-10">
        ...
        <a href="{{ route('products.index') }}" class="text-sm uppercase tracking-widest underline">Voir tout</a>
    </div>
