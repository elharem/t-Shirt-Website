<footer class="bg-ink text-cream mt-20">
    ...
    <div class="container mx-auto px-4 py-12">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            {{-- Colonne 1 : Espace client --}}
            <div>
                <h4 class="text-sm uppercase tracking-widest mb-4 font-bold">Espace client</h4>
                <ul class="space-y-2.5 text-sm text-cream/80">
                    @auth
                        <li><a href="{{ route('profile.edit') }}" class="hover:text-accent">Mon profil</a></li>
                        <li><a href="{{ route('orders.index') }}" class="hover:text-accent">Mes commandes</a></li>
                        <li>
                    ...
                    @else
                        <li><a href="{{ route('login') }}" class="hover:text-accent">Se connecter</a></li>
                        <li><a href="{{ route('register') }}" class="hover:text-accent">Créer un compte</a></li>
                        <li><a href="{{ route('cart.index') }}" class="hover:text-accent">Mon panier</a></li>
                    @endauth
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-2.5 text-sm text-cream/80">
    <li><a href="{{ route('pages.show', 'livraison') }}" class="hover:text-accent">Modes et frais de livraison</a></li>
    <li><a href="{{ route('pages.show', 'paiement') }}" class="hover:text-accent">Moyens de paiement</a></li>
    <li><a href="{{ route('pages.show', 'retours') }}" class="hover:text-accent">Retours et remboursements</a></li>
    <li><a href="{{ route('pages.show', 'rse') }}" class="hover:text-accent">Nos engagements RSE</a></li>
</ul>
            </div>
...
<div>
    ...
    <ul class="space-y-2.5 text-sm text-cream/80">
        <li><a href="{{ route('pages.show', 'contact') }}" class="hover:text-accent">Nous contacter</a></li>
        <li><a href="{{ route('pages.show', 'faq') }}" class="hover:text-accent">FAQ</a></li>
        <li><a href="{{ route('pages.show', 'guide-tailles') }}" class="hover:text-accent">Guide des tailles</a></li>
        <li><a href="{{ route('pages.show', 'about') }}" class="hover:text-accent">Qui sommes-nous ?</a></li>
    </ul>
</div>
            ...
            <div>
                ...
                <ul class="space-y-2.5 text-sm text-cream/80">
                    ...
                    <li><a href="{{ route('cookies.show') }}" class="hover:text-accent">Gérer mes cookies</a></li>
                </ul>
            </div>
        </div>

        {{-- Section marque + réseaux sociaux --}}
        <div class="border-t border-cream/10 mt-12 pt-8 grid md:grid-cols-2 gap-6 items-center">
            ...
            <div class="md:text-right">
                ...
                <div class="flex gap-3 md:justify-end">
                    <a href="https://facebook.com/sharer/sharer.php?u={{ urlencode(url('/')) }}" target="_blank" rel="noopener" aria-label="Facebook"
                    ...
                    <a href="https://twitter.com/intent/tweet?url={{ urlencode(url('/')) }}&text={{ urlencode('Découvrez TEE/SHOP') }}" target="_blank" rel="noopener" aria-label="Twitter"
                       class="w-9 h-9 border border-cream/30 flex items-center justify-center hover:bg-accent hover:border-accent transition">𝕏</a>
                    <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url('/')) }}" target="_blank" rel="noopener" aria-label="LinkedIn"
                    ...
                    <a href="https://wa.me/?text={{ urlencode('Découvrez TEE/SHOP : ' . url('/')) }}" target="_blank" rel="noopener" aria-label="WhatsApp"
    ...
    <div class="bg-ink-soft border-t border-cream/10">
        <div class="container mx-auto px-4 py-5 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-cream/60">
            <p>© {{ date('Y') }} TEE/SHOP — Projet Groupe 3 — Tous droits réservés.</p>
